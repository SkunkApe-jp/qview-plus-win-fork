...
void MainWindow::aiRename()
{
    if (!getCurrentFileDetails().isPixmapLoaded)
        return;

    auto *aiRenameDialog = new QVAIRenameDialog(this, getCurrentFileDetails().fileInfo);
    connect(aiRenameDialog, &QVAIRenameDialog::newFileToOpen, this, &MainWindow::openFile);
    connect(aiRenameDialog, &QVAIRenameDialog::readyToRenameFile, this, [this]() {
        if (auto device = graphicsView->getLoadedMovie().device()) {
            device->close();
        }
    });
